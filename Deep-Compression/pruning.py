        ...
        if hasattr(module, 'mask'):
            def make_hook(layer_name):
                def hook(_module, inputs, _output):
                    ...
                    if features is None:
                        return
                    if features.dim() > 2: # Conv layer
                        # For conv layers, we care about the input channels.
                        # We average the stats across the spatial dimensions (H, W).
                        flattened = features.detach().abs().mean(dim=[2, 3]).to(dtype=torch.float32, device='cpu') # Shape: (batch_size, in_channels)
                    else: # Linear layer
                        flattened = features.detach().abs().to(dtype=torch.float32, device='cpu') # Shape: (batch_size, in_features)
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (data, target) in pbar:
            ...
            for name, p in model.named_parameters():
                ...
                # This assumes the mask has the same name as the weight with a .mask suffix
