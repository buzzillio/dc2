            ...
            def make_hook(layer_name):
                def hook(_module, inputs, _output):
                    ...
                    if features.dim() > 2: # Conv layer
                        ...
                        flattened = features.detach().abs().mean(dim=[2, 3]).to(dtype=torch.double, device='cpu') # Shape: (batch_size, in_channels)
                    else: # Linear layer
                        flattened = features.detach().abs().to(dtype=torch.double, device='cpu') # Shape: (batch_size, in_features)
